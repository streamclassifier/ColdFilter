...
private:
	...
	BOBHash64 * bobhash1;
	BOBHash32 * bobhash2[MAX_HASH_NUM];
...
void SC::Insert(uint key)
{
	...
	__m128i e_comp = _mm_cmpeq_epi32(item, keys_p[4]);
	__m128i f_comp = _mm_cmpeq_epi32(item, keys_p[5]);
	__m128i g_comp = _mm_cmpeq_epi32(item, keys_p[6]);
	__m128i h_comp = _mm_cmpeq_epi32(item, keys_p[7]);

	e_comp = _mm_packs_epi32(e_comp, f_comp);
	g_comp = _mm_packs_epi32(g_comp, h_comp);	
	e_comp = _mm_packs_epi32(e_comp, g_comp);

	int matched1 = _mm_movemask_epi8(a_comp);
	int matched2 = _mm_movemask_epi8(e_comp);


	if(matched1 != 0)
	{
		//return 32 if input is zero;
		int matched_index = _tzcnt_u32((uint)matched1);
	...
	{
		//return 32 if input is zero;
		int matched_index = _tzcnt_u32((uint)matched2) + 16;
	...
	Insert_SC_SPA(ID[bucket_id][0], counter[bucket_id][0]);
	
	ID[bucket_id][0] = key;
	counter[bucket_id][0] = 1;
